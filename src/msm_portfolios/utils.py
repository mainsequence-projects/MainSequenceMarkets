...
from msm_portfolios.asset_scope import asset_display_name, asset_unique_identifier
...
def get_last_query_times_per_asset(
    ...
    for asset in asset_list:
        asset_id = asset_unique_identifier(asset)
        ...
        if asset_start_time >= (current_time - pd.Timedelta(query_frequency)):
            logger.info(
                "no new data for asset %s from %s to %s",
