...
class FixingRatesNode(MarketDataNode):
    ...
    def get_asset_list(self):
        ...
        for rc in self.rates_config.rates:
            ...
            payload.append({"unique_identifier": uid, "name": rc.name, "ticker": uid})
        return payload
    ...
    def update(self):
        ...
        for asset in self.get_update_asset_list() or []:
            unique_identifier = asset_field(asset, "unique_identifier")
            builder = FIXING_RATE_BUILDERS.builder_for_uid(unique_identifier)
            df = builder(
                update_statistics=self.update_statistics,
