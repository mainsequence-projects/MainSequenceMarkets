...
class ValuationResolver(Protocol):
    def __call__(
        self,
        requested_metrics: Sequence[Mapping[str, Any]],
...
def plan_account_virtual_fund_allocations(
    *,
    ...
    valuation_time: dt.datetime,
    valuation_asset_uid: uuid.UUID | str,
    requested_metrics: Sequence[Mapping[str, Any]] | None = None,
    holdings_selection_policy: HoldingsSelectionPolicy | None = None,
    valuation_resolver: ValuationResolver | None = None,
    allocation_policy: AllocationPolicy | None = None,
    source_holdings: Sequence[HoldingValuationInput | Mapping[str, Any]]
    | pd.DataFrame
    | None = None,
    direct_target_demands: Sequence[TargetQuantityDemand | Mapping[str, Any]]
    | pd.DataFrame
    | None = None,
    virtual_fund_demands: Sequence[TargetQuantityDemand | Mapping[str, Any]]
    | pd.DataFrame
    | None = None,
    ...
    """Plan account-to-virtual-fund attribution without writing rows.

    The core implementation is vectorized over source holdings and target
    demand frames. Repository-backed resolution from ``position_set_uid`` is
    intentionally kept outside this pure planner until the workflow supplies
    the portfolio-target expansion boundary.
    """

    if source_holdings is None or virtual_fund_demands is None:
        raise ValueError(
            "plan_account_virtual_fund_allocations requires resolved source_holdings "
            "and virtual_fund_demands. Resolve PositionSetTable, AccountHoldingsSetTable, "
            "TargetPositionsStorage, portfolio weights, and valuation before calling the "
            "pure planner."
        )
    ...
    if valuation is not None:
        diagnostics.extend(valuation.diagnostics)
    ...
    return _plan_from_quantity_demands(
        ...
        source_holdings=holdings,
        direct_target_demands=direct_demands,
        virtual_fund_demands=virtual_demands,
...
def _requested_metric_names(
    requested_metrics: Sequence[Mapping[str, Any]] | None,
) -> set[str]:
    if requested_metrics is None:
        return {"nav"}
    return {
        _required_string(metric.get("name"), "requested_metrics[].name")
