"""Derive TimeIndexTableUpdater frame-validation schema from storage-first metatables.
...
source of truth, ADR 0017) so TimeIndexTableUpdater validators never duplicate that schema.
...
def storage_column_dtypes_map(output_table: Any) -> dict[str, str]:
    ...
    return {
        ...
        for column in output_table.__table__.columns
    }


def storage_column_nullable_map(output_table: Any) -> dict[str, bool]:
    """Map each storage column name to its SQLAlchemy nullable flag."""

    return {column.name: bool(column.nullable) for column in output_table.__table__.columns}


def storage_index_names(output_table: Any) -> list[str]:
    """Return the storage class' declared index names."""

    return list(output_table.__index_names__)


def storage_time_index_name(output_table: Any) -> str:
    """Return the storage class' declared time index column name."""

    return output_table.__time_index_name__
