"""Provider-neutral Command Center resource collection contracts."""
...
class ResourcePageInfo(HttpContractModel):
...
class ResourceCollection(HttpContractModel, Generic[ResourceT]):
...
def resource_collection_response(
    *,
    ...
    """Build a collection at a FastAPI boundary and expose invalid windows as HTTP 400."""
...
class ResourceSearchControl(HttpContractModel):
...
class ResourceFilterOption(HttpContractModel):
    value: FilterValue
    label: str = Field(pattern=r".*\S.*")


class ResourceTextFilter(HttpContractModel):
    key: str = Field(pattern=r".*\S.*")
    label: str = Field(pattern=r".*\S.*")
    type: Literal["text"] = "text"


class ResourceBooleanFilter(HttpContractModel):
    key: str = Field(pattern=r".*\S.*")
    label: str = Field(pattern=r".*\S.*")
    type: Literal["boolean"] = "boolean"


class ResourceSelectFilter(HttpContractModel):
    key: str = Field(pattern=r".*\S.*")
    label: str = Field(pattern=r".*\S.*")
...
class ResourceListControls(HttpContractModel):
...
class ResourceIdentity(HttpContractModel):
    fields: list[str] = Field(min_length=1)
...
class ResourceDescriptor(HttpContractModel):
...
class ResourceColumn(HttpContractModel):
    id: str = Field(pattern=r"^[a-z][a-z0-9-]*$")
...
class ResourceListDiscovery(HttpContractModel):
...
class ResourceDiscovery(HttpContractModel):
