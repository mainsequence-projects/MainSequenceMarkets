...
from examples.msm.assets.utils import EXAMPLE_BTC_ASSET, EXAMPLE_CRYPTO_ASSET_TYPE
...
def run_account_workflow() -> dict[str, Any]:
    ...
    print("2. Registering the BTC Asset that holdings and targets reference.")
    asset = Asset.upsert(**EXAMPLE_BTC_ASSET)
    print(f"   Asset uid={asset.uid} unique_identifier={asset.unique_identifier}")

    print("3. Registering one reusable AccountModelPortfolio.")
    ...
    print("4. Registering the AccountGroup that groups both accounts.")
    ...
    print("5. Creating two accounts and assigning both to the group.")
    accounts = [
    ...
    print("6. Creating the AccountHoldings DataNode instance.")
    ...
    print(f"7. Using one UTC workflow timestamp: {workflow_time.isoformat()}")

    print(
        "8. Creating account-owned target portfolio relationships that both "
    ...
    for account in accounts:
                ...
                **EXAMPLE_ACCOUNT_TARGET_PORTFOLIO,
                ...
                "metadata_json": {
                    **(EXAMPLE_ACCOUNT_TARGET_PORTFOLIO.get("metadata_json") or {}),
                    "account_unique_identifier": account.unique_identifier,
                },
            }
        )
        print(
        ...
        position_set = PositionSet.upsert(
            ...
            metadata_json={"account_unique_identifier": account.unique_identifier},
        )
        print(
            "   PositionSet "
    ...
    print("9. Building holdings rows for both accounts.")
    holdings_frames = []
    for account, quantity in zip(accounts, (10.0, 5.0), strict=True):
        ...
        print(
            ...
            f"{account.unique_identifier}: quantity={quantity} rows={len(holdings_frame)}"
        )
    ...
    print("10. Running the AccountHoldings DataNode update.")
    ...
    print("11. Pretty-printing resolved positions for each account.")
